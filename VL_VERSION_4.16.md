# VL Syntax Reference 4.16 (Rules & Widgets)

> **Public specification release**: 4.16
> **Executable VL language level**: 4.4.3
> **Compatibility**: VL source files must continue to use `// VL_VERSION:4.4.3` until a parser release explicitly introduces a `4.16` language level.
>
> The public release number unifies the VL and Workflow documentation set. It does not invent a new parser mode or change the syntax rules described below.
...
Current version: `// VL_VERSION:4.4.3`
...
├─ Config/                # Project-level non-secret environment bindings; do not participate in VL compilation
│   └─ env.json
...
`Config/` is the standard project-level non-source configuration directory. It does not participate in VL compilation, does not define business VL syntax, does not replace `Services/Sections/ExtComponents/Apps`, and must not be used as the source of online platform deployment identity.

| File | Writable | Maintainer | Purpose |
| ---- | -------- | ---------- | ------- |
| `Config/env.json` | Yes | user / AI agent | Project-level non-secret environment values and secret reference bindings |

**Standard responsibilities:**

- `env.json`: project-level environment binding document. It may contain non-secret literal environment values and references to tenant-level secrets, but MUST NOT contain plaintext secret values
- Platform deployment identity, including platform group ids, work ids, node ids, preview targets, production targets, publish targets, launch targets, workspace bindings, and app-to-work deployment snapshots, is external platform deployment context and is not VL source syntax
- Tool-owned deployment state may be maintained by VLCode IDE, agent, CLI, or platform deployment systems outside the VL language contract
- `Config/env.json` is the only legal file in the current `Config/` project contract. Files such as `Config/project.state.json`, `Config/project.settings.json`, `Config/secrets.json`, or any other `Config/*` file are not legal current-spec project files and must not be read as VL source, deployment identity, or secret storage.

#### `Config/env.json`

`Config/env.json` is an optional project-level environment binding file. It is a configuration artifact, not a VL source file. It may be consumed by IDE, AI-agent, compile, or deployment tooling to write runtime environment configuration, but it does not itself create VL declarations. VL code must still declare the variables and secrets it uses.

Top-level keys are deployment environment names such as `local`, `preview`, and `prod`. Each environment entry may contain `backend` and frontend app entries. Backend entries are split into `env` and `secrets` to match the VL declaration sections:
...
```json
{
  "preview": {
    "backend": {
      "env": {
        "WECOM_CORP_ID": "ww5a02b3c65bdef689",
        "WECOM_AGENT_ID": "1000019",
        "WECOM_CALLBACK_URL": "https://pre.dev.h5app.com/play/xxx/route/1?oauthProvider=wecom"
      },
      "secrets": {
        "WECOM_CORP_SECRET": {
          "secretRef": "wecom1.corpSecret"
        },
        "WECOM_STATE_SECRET": {
          "secretRef": "oauthState.wecom1"
        }
      }
    }
  }
...
- `backend.env` contains non-secret literal values only
- `backend.secrets` contains secret reference bindings only; each value MUST be an object with a non-empty `secretRef` string
- `secretRef` identifies a tenant-level Secret Manager entry; it is not the secret value
- Plaintext secret values MUST NOT appear in `Config/env.json`
- Project export may include `Config/env.json` and secret references, but MUST NOT include resolved secret values
- Compile/deployment tooling may resolve these bindings into runtime configuration; parser grammar MUST NOT treat `Config/env.json` as a replacement for `ENV` or `SECRET` declarations in `.vs`

### Platform Deployment Context

Platform deployment context is external to VL source.

VL source describes the application: pages, sections, components, services, database declarations, workflow, theme, public interfaces, environment declarations, and secret dependencies. It must not describe online platform deployment identity.

The following values are platform deployment context, not VL syntax:

- platform group id / work group id
- frontend or backend work id / node id
- preview, production, publish, or launch target
- region selection used by platform APIs
- current workspace / tenant / account binding
- local rustbase endpoint used for local execution
- any deployment snapshot that binds `Apps/*.vx` or backend output to platform works

Parser, IDE, agent, CLI, or platform deployment tools MAY receive this information through request parameters or an external tool-owned deployment state, for example:
...
```json
{
  ...
  "platformContext": {
    ...
    "backendNid": 456,
    "frontendNids": {
      "Apps/AdminApp.vx": 789
    },
    "rustbaseUrl": "http://127.0.0.1:3000"
  }
}
```

This example is a parser/tooling contract shape, not VL source syntax. Exact field names may be refined by Platform API documentation, but the boundary is mandatory: platform identity is passed from outside the VL project source.

When `target` is `local` or omitted and local rustbase is available, a VL project must be runnable without online platform binding. Online preview, production publish, launch, or platform synchronization may require external platform context, but that requirement is a deployment request requirement, not a VL language requirement.
...
-$isLoading = false
-FAILIF (!_result.success) "Load products failed"
...
- `// VL_WC_LEVEL:3` is the current standard authoring level for `VL_VERSION:4.4.1` `.wc` source.
...
- Levels `1` and `2` are reserved for historical parser compatibility and must not be generated for new `VL_VERSION:4.4.1` source.
...
**Required Sections**: `# Backend Tree`, `# Services`. Optional sections: `# Backend Environment Vars`, `# Backend Secrets` (both before `# Backend Tree`).
...
ENV SOME_API_BASE_URL(STRING) "External API base URL"

# Backend Secrets
SECRET SOME_API_KEY "External API secret key"
...
```vl
// Preview: width:240px height:600px
<Section-Sidebar "root"> containerType:col padding:16px
```

#### Closed CSS Property Model (VL 4.3.8)
...
##### Skeleton Size Value Units

VL size properties use a closed value-unit model. A parser or lint implementation MUST NOT silently ignore an unsupported CSS unit on a recognized skeleton property. If a value unit is unsupported for the current property and node scope, compilation reports an error.

General frontend size values:

1. `px`, `%`, numeric zero, and the existing keyword values already defined for a property remain valid according to that property's normal rules.
2. `vw`, `vh`, `dvw`, and `dvh` are supported only as viewport-shell size units.

Viewport-shell size units:

1. Supported units: `vw`, `vh`, `dvw`, `dvh`.
2. Supported properties: `width`, `min-width`, `max-width`, `height`, `min-height`, `max-height`.
3. Supported node scope: frontend root or page-shell layout only.
   - `<App-*>`, `Page-*`, and `Modal-*` boundary nodes
   - the real root container of a `.vx`, `.sc`, or `.cp` frontend source
   - a direct first-level `Row`, `Col`, `Grid`, or `Block` whose parent is one of the frontend roots or boundary nodes above
   - under a page-level boundary (`<App-*>` / `<Page-*>` / `<Modal-*>`) only, a second-level `Row`, `Col`, `Grid`, or `Block` whose parent is such a first-level layout shell — i.e. the `Page -> Row/Col/Grid/Block shell -> Row/Col/Grid/Block sidebar/main` pattern. This second-level allowance does NOT apply under `.sc` / `.cp` reuse-module roots, which stay first-level only.
4. Unsupported scope: third-level and deeper layout containers, a second-level node that is not a layout container, ordinary nested containers, atomic components, table cells, buttons, text nodes, images, form controls, and module instance usage sites (`Section-*`, `Component-*`, `WebComponent-*`) must not use viewport units.
5. `dvh` / `dvw` are preferred for dynamic viewport sizing when the target runtime may have browser chrome that changes visible viewport size.
6. For full-screen page shells, prefer minimum viewport constraints over forced width where possible: `<Col-ViewportShell "shell"> min-width:"100vw" min-height:"100dvh"`.

Examples:

- Valid page shell: `<Col-ViewportShell "shell"> min-width:"100vw" min-height:"100dvh"`.
- Valid page-level sidebar/main (second level under a `Page` shell): `<Page-Board "board"> path:"b"` -> `<Row-Shell "shell"> min-height:"100vh"` -> `<Col-Sidebar "sidebar"> min-height:"100vh"`.
- Invalid third-level layout: a `Col` nested under that sidebar/main (`Page -> Row shell -> Col sidebar -> Col inner`) must not use viewport units.
- Invalid ordinary text node: `<Text-Title "title"> width:"100vw" value:"Title"`.
- Invalid module instance usage: `<Section-LoginCard "login"> width:"100vw"`.
...
| Property | Layout container | Text/Icon | Button | Input/Textarea | Image/Video | Divider | Module instance |
|---|---|---|---|---|---|---|---|
| `sk.bg` `sk.bgImage` `sk.shadow` `sk.radius` `sk.radiusTopLeft/TopRight/BottomRight/BottomLeft` `sk.opacity` | ✅ | ✅ | ✅ | ✅ | ✅ (radius mainly) | ❌ | ✅ |
| `sk.borderColor` `sk.borderWidth` `sk.borderStyle` `sk.borderTop/Right/Bottom/Left` | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| `sk.fg` | ❌ | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ |
...
- Surface family (`sk.bg`, `sk.bgImage`, `sk.shadow`, `sk.radius`, `sk.radiusTopLeft/TopRight/BottomRight/BottomLeft`, `sk.opacity`) acts on the node's host element and is allowed on every non-Divider node (Divider's body is a single line, no fillable surface)
- Border family (`sk.borderColor`, `sk.borderWidth`, `sk.borderStyle`, `sk.border*`) is allowed on every node (Divider's border IS the divider)
...
1. Explicit size declaration (e.g., `height:"400px"`, `width:"320px"`)
...
4. A viewport-shell size value, but only when the scroll container itself is in the viewport-shell scope defined by `Skeleton Size Value Units`
...
`# Frontend Tree` and `# Backend Tree` are declaration-only structure sections. They may contain component instance declarations and legal tree nesting, but they must not contain event-handler listener definitions, executable action calls, variable assignments, local variable declarations, `IF` / `FOR` / `WHILE`, `FAILIF`, `GUARD`, `RETURN`, or rollback statements.

`# Backend Environment Vars` and `# Backend Secrets` are declaration-only backend configuration sections. They must be placed before `# Backend Tree` in `.vs` files. `# Backend Environment Vars` declares non-secret backend runtime variables using `ENV NAME(TYPE) "description"`. `# Backend Secrets` declares backend-only secret dependencies using `SECRET NAME "description"`. Neither section may contain executable statements or default values.

Rules:

- `ENV` declarations describe non-secret runtime configuration that may be backed by `Config/env.json` `backend.env` literal values or platform environment configuration
- `SECRET` declarations describe backend-only secret dependencies that must be backed by Secret Manager entries through `Config/env.json` `backend.secrets` reference bindings or equivalent platform configuration
- `SECRET` names share the same identifier namespace as backend `ENV` names within the same ServiceDomain; duplicate names are invalid
- `SECRET` declarations MUST NOT include plaintext values, default values, or inline `secretRef` values
- Frontend files and frontend sections MUST NOT declare or access backend secrets
- Backend executable logic reads both backend environment variables and resolved backend secrets through `SYSENV.NAME`
...
- For `.sc/.cp`, the root container type is controlled by `containerType`; default is `col`, and `row` / `grid` are also allowed in `VL_VERSION:4.4.1`.
...
For `.sc/.cp`, `VL_VERSION:4.4.1` uses the real root-container model described in this document, and `containerType` defaults to `col`.
...
| SYSENV.currentApp  | Frontend and Backend                   | Runtime app information that is safe for VL business code, such as **playPath**: STRING (current app play path), **previewDomain**: STRING (current app preview domain), and **publishDomain**: STRING (current app publish domain) when provided by the current runtime. Platform deployment ids are not part of the VL authoring contract and must not be required by source code. |
...
| SYSENV.<NAME>      | Backend Only for backend ENV and SECRET declarations | Reads a resolved backend runtime value declared in `# Backend Environment Vars` or `# Backend Secrets`. ENV values come from non-secret environment configuration. SECRET values come from Secret Manager resolution and must not be exposed to frontend code, exported project content, logs, or user-facing error messages. |
...
#### Bare Variable Statements Should Be Resolved

In executable logic blocks, a line whose whole statement is only a variable reference has no executable effect and should be treated as an unfinished intent.

Warning examples: `-$onToolbarAction`; `-_result`

Use one explicit action instead: `-$lastToolbarActionId = id`; `-HandleToolbarAction(id)`; `-@toolbarAction(id)`

This rule applies to event handlers, methods, services, transactions, and nested executable blocks under `IF`, `FOR`, `WHILE`, or `FAILIF`. Lint should report a warning, not a compile-blocking error, and the warning must include concrete repair suggestions. If the intent is to record state, use assignment. If the intent is to run business logic, call a method or service. If the intent is to notify the parent module, trigger a declared public event.
...
When handling a child component public event, the parent handler must choose an explicit effect:

1. Record state, e.g. `-$lastToolbarActionId = id`
2. Run local logic, e.g. `-HandleToolbarAction(id)`
3. Forward the event, e.g. `-@toolbarAction(id)`

Do not use `$onXxx` variables as callback placeholders. If a handler only needs to remember the latest event, prefer state names such as `$lastToolbarActionId`, `$lastFormSubmit`, or `$selectedActionId`. For object state, assign an object explicitly, e.g. `-$lastFormSubmit = {id:id}`.
...
`FAILIF` is a dedicated fail-fast statement for high-frequency defensive checks and failure handling blocks.
...
- When `condition == true`, the fail-fast branch is triggered
- When `condition == false`, execution continues
- `FAILIF` does not replace `IF`; it is only for "fail and exit now" semantics
- Child statements under `FAILIF` are failure-handling statements, such as showing an error message, compensation, or logging
...
- `-FAILIF (!_result.success) _result.message`
- `-FAILIF (_result.dataArray.length == 0) "Course list is empty"`
- `-FAILIF (_courseList.dataArray.length == 0) "Course list is empty"`

A child body under `FAILIF` may contain failure-side statements such as showing a toast, compensation, or logging.

If a `FAILIF` block contains no explicit `RETURN` / `ROLLBACK`, the default host exit behavior still applies after the block body finishes.

`GUARD condition "message"` remains a compatibility alias for `FAILIF condition "message"` and has identical runtime semantics. New authored VL should use `FAILIF` so the condition is read as a failure condition rather than a positive assertion.
...
> Border governance rule: in VL, the full border triad (width/style/color) and any border shorthand are managed as skin. Border CSS literals must not be authored in SC/CP code. Use Theme slots (`surfaceBorder*`, `surfaceDivider*`), `Divider` with style coordinate, or `sk.borderColor` / `sk.borderWidth` / `sk.borderStyle` / `sk.borderTop` / `sk.borderRight` / `sk.borderBottom` / `sk.borderLeft` for component-local skin override.
...
`style` is a component attribute written outside `<>`. Its value **must be a static string literal**; VL expressions, variables, and ternary operators are forbidden. The value is a `|`-separated string; each word uniquely resolves to a static dimension point. Order does not matter. Business/data-condition visual changes MUST NOT be expressed by ternary `style` values; use direct-child `StateStyle` nodes with `conditions:` and static `style:"..."` coordinate patches. Component-local `sk.*` props are only an escape hatch for narrow skin values that the style space cannot yet express; AI/tools MUST NOT replace ordinary conditional style switching with many conditional `sk.*` bindings.
...
3. The point lists below are the **complete legal point sets** for VL 4.3.8. Any dimension name or point name not listed here is invalid. AI/tools **must not** invent aliases, synonyms, or new point names.
...
This priority chain describes final CSS precedence, not the preferred authoring path. For state-dependent semantic styling, prefer `StateStyle conditions + style:"..."`; use `sk.*` only for narrow component-local skin exceptions.
...
Purpose: Container layer, backdrop material, and decoration. In VL 4.3.8 it primarily applies to containers and the upgraded Button baseline. Input / Text / Icon do not enable `surface`; Divider is a structural visual element and uses `surface`.
...
**Relationship with VL StateStyle widget:** The runtime-state model is separate from `StateStyle`. VL code `StateStyle` widgets with `conditions:` are for business-logic conditional style switches (e.g., `conditions:$isHighlighted`), but they are not a dynamic skin value system. Business-condition skin changes should use `StateStyle + style` coordinate patches whenever the result maps to legal style-space points; `sk.*` remains a local escape hatch for narrow values the style space cannot yet express.
...
Each component type has a platform-defined set of supported static style dimensions. Dimensions declared in `style` must be within that set; otherwise a compile error is reported.
...
4. `state` point appears in `style` → error
5. `style` value contains VL expression, variable, function call, or any non-literal content (e.g., `style:($cond ? "success" : "danger")`, `style:$buttonStyle`, `style:computeStyle()`) → error (STY-001). If the intended result is conditional visual switching, use `StateStyle conditions + style:"..."` instead.
...
1. Theme file validity and rule matching are governed by the `VL_VERSION` declared in the file's first-line comment (e.g., `// VL_VERSION:4.4.1`)
...
| `sk.borderStyle` | `border-style` | Border line style |
...
| `sk.radiusTopLeft` | `border-top-left-radius` | Top-left corner radius |
| `sk.radiusTopRight` | `border-top-right-radius` | Top-right corner radius |
| `sk.radiusBottomRight` | `border-bottom-right-radius` | Bottom-right corner radius |
| `sk.radiusBottomLeft` | `border-bottom-left-radius` | Bottom-left corner radius |

- Total 17 properties, platform-level fixed definitions.
- Developers cannot create custom `sk.*` property names; using undefined `sk.*` properties MUST trigger a compile error.
- The whitelist is fixed at 17 properties; allowing different literal forms does **not** expand the property set.
...
| `sk.radiusTopLeft:"5px"` | Static string literal | ✅ Allowed |
...
| `border-style:"dashed"` | ❌ Compile error, suggest using `sk.borderStyle` |
| `border-top-left-radius:"5px"` | ❌ Compile error, suggest using `sk.radiusTopLeft` |

Skin property name blocklist (compiler intercepts): `background-color`, `background-image`, `background`, `color` (when used for text color), `border`, `border-top`, `border-right`, `border-bottom`, `border-left`, `border-color`, `border-width`, `border-style`, `box-shadow`, `opacity`, `border-radius`, `border-top-left-radius`, `border-top-right-radius`, `border-bottom-right-radius`, `border-bottom-left-radius`, `text-transform`, `cursor`.
...
3. **Input / Text / Icon do not support surface dimension.** Button supports `surface` in VL 4.3.8; input and text-like atomic components still do not.
...
| 30 | SC/CP code contains border literal properties (`border`, `border-top`, `border-bottom`, `border-left`, `border-right`, `border-color`, `border-style`, `border-width`, `border-radius`, `border-top-left-radius`, `border-top-right-radius`, `border-bottom-right-radius`, `border-bottom-left-radius`) outside style-space resolution. In VL 4.3.8 this is also covered by the closed CSS-property whitelist / skin blocklist rules. | error |
...
| 49 (STY-001) | `style` value is not a static string literal | error |
...
| 63 (BST-001) | `BackendSecurityToolkit` declared outside `# Backend Tree` | error |
| 64 (DB-SM-001) | `.vdb` Database or Table declares `schemaMode` other than `managed` or `bindOnly` | error |
| 65 (DB-SM-002) | bindOnly table declares `data` seed records | error |
| 66 (DB-SM-003) | bindOnly table alias/name does not exist in the target project or target group | error |
| 67 (DB-SM-004) | bindOnly table declares a field that is missing from, or type-incompatible with, the existing platform table | warning |
...
- `StateStyle conditions + style:"..."` is the preferred path for conditional static coordinate switching. It is not a dynamic skin value system; a `StateStyle` node may still use legal `sk.*` props as a narrow local escape hatch.
...
`.vdb` supports `schemaMode` at both the `Database` root and each `Table` declaration. Valid values are `managed` and `bindOnly`.

- Default mode is `managed`.
- Root `schemaMode` defines the default mode for all tables in the `.vdb` file.
- Root `bindOnly:true` remains a compatibility alias for root `schemaMode:bindOnly`; `schemaMode:bindOnly` is the canonical spelling for new `.vdb` source.
- Table `schemaMode` overrides the root default for that table.
- A single `.vdb` file MAY mix managed tables and bindOnly tables.
- Canonical root example: `<Database-Db> schemaMode:managed`.
- Canonical table examples: `<Table-LegacyUsers> schemaMode:bindOnly`; `<Table-NewOrders> schemaMode:managed`.
- Invalid `schemaMode` values are compilation errors.

`managed` tables are owned by parser deployment: parser may create or sync table structure, columns, indexes, relation metadata, and declared seed data according to the normal database deployment rules.

The effective declaration mode of a table is calculated only from the `.vdb` declaration: root default (`schemaMode` or compatible root `bindOnly:true`) plus table-level `schemaMode` override.

`bindOnly` tables are bindings to existing platform database tables:

- The table alias/name MUST already exist in the target project or target group; missing alias/name is `DB-SM-003` error.
- Declared fields are binding/type annotations only; parser MUST NOT create missing fields for a bindOnly table.
- A declared field that is missing from the existing platform table, or whose declared type is incompatible with the platform table field, is `DB-SM-004` warning. Parser MUST NOT create or alter the field during bindOnly deployment.
- Declared indexes are annotations only; parser MUST NOT create or update indexes for a bindOnly table.
- `data` seed declarations are forbidden on tables whose effective `.vdb` declaration mode is bindOnly; this is `DB-SM-002` error.
- Parser MUST NOT reset, clear, import seed data into, or otherwise mutate data for a bindOnly table.
- Relations touching a bindOnly table are binding metadata only; parser MUST NOT create or update platform relation records for those relations.
- Relations whose source and target tables are both managed may still be created or synced normally.

Request-level `dbSchemaMode:"bindOnly"` or `bindOnly:true` is a parser execution safety mode, not a `.vdb` declaration-mode rewrite. When this mode is supplied by the parse request, all `.vdb` tables are treated as read-only for that parse run; table-level `schemaMode:managed` MUST NOT make any table mutable in that run. Request-level bindOnly does not trigger `DB-SM-002` for a managed table that legally declares seed `data`; parser skips seed/schema/data writes for that run. Request-level bindOnly combined with `resetData` / `resetDbData` remains a runtime deployment error.

Any bindOnly preflight error, including `DB-SM-003`, MUST stop the entire database deployment before any table schema or data write occurs. This atomic stop applies even when the `.vdb` mixes managed and bindOnly tables. Preflight warnings such as `DB-SM-004` do not stop deployment.
...
-<Field-user_id> type:INT notNull:true
...
2. `Table-AuthUserIdentities` MUST declare identity credential fields (at minimum: `user_id(INT)`, `identity_type(STRING)`, `identifier(STRING)`, `identifier_norm(STRING)`, `credential_hash(STRING)`, `provider(STRING)`, `provider_user_id(STRING)`, `provider_union_id(STRING)`, `verified_at(TIMESTAMP)`, `is_primary(BOOL)`, `extra_json(JSON)`)
...
If at runtime the `flex:"1"` height carrier chain remains unstable, explicit size carrier may be used while the parent chain is corrected, for example:

```vl
<Col-List> height:"480px" overflow:"auto"
```

Note: Internal scroll areas should prefer returning to a semantic size carrier chain. Viewport units are reserved for viewport-shell scope and must not be used as a shortcut inside ordinary nested scroll containers.
...
-RETURN {success:_result.success,message:_result.message}
```
...
-FAILIF (!_loginResult.success) _loginResult.message
...
-FAILIF (!_tokenResult.success) _tokenResult.message
...
### 11. Executable Statements Must Have Explicit Effects

- **MUST**: Event handlers, methods, services, transactions, and executable child blocks must use explicit-effect statements: assignment, method call, component/system method call, public event trigger, control flow, `RETURN`, `CUSTOM_RETURN`, `ROLLBACK`, or `FAILIF`.
- **SHOULD NOT**: A bare `$variable`, `_variable`, `$object.field`, or `_object.field` line should not be used as a placeholder for callback, state recording, or future logic; lint should warn because the line has no executable effect.
- **MUST**: If AI generation cannot determine the business action for a child component event, it may record the event state with a typed assignment or omit the extra action; it should not emit a no-op variable reference.
- **MUST**: The lint warning for a bare variable statement must include a concrete fix suggestion, such as assigning the event parameter to state, calling a local method, or triggering a declared public event.
...
Frontend system utility component for common client-side operations (timing, logging, browser-level page actions) and system-level user refresh.
...
* `alertLog(info)`: Shows a browser alert-style debug message. Use it only for simple debugging; use `SysUI.showModal` or `SysUI.showToast` for product UI.

**Browser Page Actions**

* `openLink(url, currentWindow)`: Opens an external link or absolute URL. Parameters: `url(STRING)` - target URL, `currentWindow(BOOL)` - when `true`, open in the current browser window; when `false`, open in a new browser window or tab. This is the VL-facing method name from `locale.en`; the underlying platform action is `openUrl`. Use this method for cross-application or external full-page navigation, such as jumping from a business project to the user center login page. Use `SysRoute.navigate(...)` for in-app route navigation.
* `reload()`: Reloads the current browser page. This is the VL-facing method name from `locale.en`; the underlying platform action is `refresh`.
* `changeFavicon(ico)`: Changes the current page favicon. Parameter: `ico(STRING)` - favicon URL or icon resource string.
...
* `browseImages(maxNumberPic, outputBase64, compressedWidth, compressedHeight, type, encoderOptions)`: Read multiple local images
  ...
  - Return Value:
    ...
    - `failureReason`: **Read failure reason** (Reason for failed image reading.)
* `browseFile(outputBase64, accept, outputText)`: Read local file
  - Description: Reads one local file's info, returns name, type, size, temporary local path, base64 data, text content, etc. Not supported in WeChat browser, webApp WeChat mini-program, Alipay/DingTalk mini-programs.
  - Parameters:
    ...
    - `outputText(Boolean)`: **Read as text** (Default no. When yes, reads the selected file as UTF-8 text and returns it in the `text` field. A leading UTF-8 BOM is removed automatically.)
  - Return Value:
    - `name`: **Name** (File name.)
    - `type`: **Type** (File type.)
    - `size`: **Size** (File size.)
    - `blobUrl`: **Temporary local path** (File temporary local path.)
    - `base64Code`: **base64 data** (File's base64 format data. Note: Must choose base64 format during reading to access. Can be returned together with `text` when both `outputBase64` and `outputText` are enabled.)
    - `text`: **Text content** (File's UTF-8 text content. Note: Only has result when `outputText` is enabled during reading. Intended for UTF-8 text files such as TXT, CSV, JSON, and similar plain-text formats.)
...
- Boundary rule: `conditions` controls activation only. `StateStyle` is a static style switch, not a dynamic value binding system. Business-condition skin changes should use `StateStyle + style` coordinate patches whenever the result maps to legal style-space points. `sk.*` remains a local escape hatch for narrow values the style space cannot yet express, and should not be used as a bulk replacement for conditional style coordinates.
...
| `insertMany`      | source([{}])                                  | success, message, dataIds              | Insert multiple records, up to 1000 rows, returns inserted IDs |
...
# Insert multiple records - returns inserted IDs
<VirtualTable-Users "userTable">.insertMany([{name:"John Doe", email:"john@example.com"}, {name:"Jane Doe", email:"jane@example.com"}]) -> _result
...
#### BackendSecurityToolkit (Backend Security Primitives)

Backend functional component for platform-provided security primitives. It is intended for deterministic, stateless backend operations that should not expose provider SDK details or credentials to project authors.

New projects should prefer `BackendSecurityToolkit` / `server-security` for backend security work. The legacy `Encryption` / `data-encrypt` entry remains a compatibility entry only and is not the recommended component for new VL code.

**Declaration:**

`BackendSecurityToolkit` is a backend functional component and MUST be declared in `# Backend Tree`.
...
New encryption and decryption methods support these encodings: `"raw"`, `"base64"`, `"base64url"`, and `"hex"`.

- ciphertext defaults to `"base64"`
- plaintext defaults to `"raw"`
- AES key defaults to `"base64"`
- AES nonce defaults to `"base64"`
- AES encryption returns the nonce together with the ciphertext when the nonce is generated automatically
- if an output encoding is `"raw"` but the output bytes are not valid UTF-8, the method must fail; binary output should use `"base64"`, `"base64url"`, or `"hex"`

* `sha256(text, outputEncoding, requestId)`: Computes the SHA-256 digest of `text`. **Return**: `success(BOOL), hash(STRING), message(STRING), status(INT), data({})`
* **text**: Plain text input to hash.
* **outputEncoding**: Output encoding. Supported values are `"hex"` and `"base64url"`.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `health(requestId)`: Checks that the backend security toolkit bridge is reachable. **Return**: `success(BOOL), name(STRING), runtime(STRING), version(STRING), ops([STRING]), message(STRING), status(INT), data({})`
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `base64UrlEncode(text, padded, requestId)`: Computes the base64url encoding (RFC 4648 §5, URL-safe alphabet using `-` and `_`) of `text`. **Return**: `success(BOOL), encoded(STRING), message(STRING), status(INT), data({})`
* **text**: Plain text input to encode.
* **padded**: Whether to include `=` padding. Default `false` (unpadded, matches the JWT convention).
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `base64UrlDecode(encoded, requestId)`: Decodes a base64url string back to UTF-8 text. Tolerant of both padded and unpadded input (trailing `=` is stripped before decoding). **Return**: `success(BOOL), decoded(STRING), message(STRING), status(INT), data({})`
* **encoded**: base64url-encoded input. Both `-`/`_` URL-safe alphabet and `+`-free padding are accepted.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `hmacSha256(data, key, outputEncoding, requestId)`: Computes HMAC-SHA-256 of `data` with `key`. **Return**: `success(BOOL), mac(STRING), message(STRING), status(INT), data({})`
* **data**: Payload string to authenticate (raw bytes).
* **key**: HMAC key string (raw bytes). Production secrets should come from backend `SECRET` declarations resolved through `SYSENV`.
* **outputEncoding**: `"hex"` (default), `"base64"`, or `"base64url"`.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `createPkceChallengeS256(verifier, requestId)`: Computes the OAuth 2.0 PKCE S256 code challenge for a verifier (RFC 7636 §4.2). The verifier MUST be 43–128 characters from the alphabet `[A-Z a-z 0-9 - . _ ~]`. **Return**: `success(BOOL), challenge(STRING), method(STRING), message(STRING), status(INT), data({})`
* **verifier**: PKCE verifier string. Use `createPkceVerifier` to generate one with a secure RNG.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `base64Encode(text, padded, requestId)`: Standard base64 encoding (RFC 4648 §4, `+` and `/` alphabet). **Return**: `success(BOOL), encoded(STRING), message(STRING), status(INT), data({})`
* **text**: Plain text input to encode.
* **padded**: Whether to include `=` padding. Default `true` (canonical standard base64).
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `base64Decode(encoded, requestId)`: Decodes a standard base64 string back to UTF-8 text. Tolerant of padded and unpadded input. **Return**: `success(BOOL), decoded(STRING), message(STRING), status(INT), data({})`
* **encoded**: base64-encoded input.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `hexEncode(text, requestId)`: Lowercase hex encoding of text bytes. **Return**: `success(BOOL), encoded(STRING), message(STRING), status(INT), data({})`
* **text**: Plain text input to encode.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `hexDecode(encoded, requestId)`: Decodes a hex string back to UTF-8 text. Accepts uppercase or lowercase. **Return**: `success(BOOL), decoded(STRING), message(STRING), status(INT), data({})`
* **encoded**: Hex-encoded input.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `urlEncode(text, requestId)`: RFC 3986 percent-encoding for generic URL components. Preserves the unreserved set `[A-Z a-z 0-9 - . _ ~]`; everything else is `%HH`. **Return**: `success(BOOL), encoded(STRING), message(STRING), status(INT), data({})`
* **text**: Plain text input to percent-encode.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `urlDecode(encoded, requestId)`: Decodes a percent-encoded string back to UTF-8 text. **Return**: `success(BOOL), decoded(STRING), message(STRING), status(INT), data({})`
* **encoded**: Percent-encoded input.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `formUrlEncode(pairs, requestId)`: `application/x-www-form-urlencoded` serialization. Spaces become `+`; everything else outside the RFC 3986 unreserved set is percent-encoded. Pair order is preserved (required for OAuth signature base strings). **Return**: `success(BOOL), encoded(STRING), message(STRING), status(INT), data({})`
* **pairs**: Ordered array of `[key, value]` tuples; both must be strings.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `sha1(text, outputEncoding, requestId)`: SHA-1 digest of `text`. **Provided for compatibility with legacy provider signatures only**; do not use for new security-sensitive workflows. **Return**: `success(BOOL), hash(STRING), message(STRING), status(INT), data({})`
* **text**: Plain text input to hash.
* **outputEncoding**: `"hex"` (default), `"base64"`, or `"base64url"`.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `md5(text, outputEncoding, requestId)`: MD5 digest of `text`. **Provided for compatibility with legacy provider signatures only**; do not use for new security-sensitive workflows. **Return**: `success(BOOL), hash(STRING), message(STRING), status(INT), data({})`
* **text**: Plain text input to hash.
* **outputEncoding**: `"hex"` (default), `"base64"`, or `"base64url"`.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `hmacSha1(data, key, outputEncoding, requestId)`: Computes HMAC-SHA-1 of `data` with `key`. Provided for compatibility with legacy provider signatures; prefer `hmacSha256` for new workflows. **Return**: `success(BOOL), mac(STRING), message(STRING), status(INT), data({})`
* **data**: Payload string to authenticate (raw bytes).
* **key**: HMAC key string (raw bytes). Production secrets should come from backend `SECRET` declarations resolved through `SYSENV`.
* **outputEncoding**: `"hex"` (default), `"base64"`, or `"base64url"`.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `randomBytes(length, outputEncoding, requestId)`: Cryptographically secure random bytes encoded as string. **Return**: `success(BOOL), value(STRING), length(INT), message(STRING), status(INT), data({})`
* **length**: Byte count, 1..=4096. Default 32 (~256 bits).
* **outputEncoding**: `"hex"` (default), `"base64"`, or `"base64url"`.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `randomString(length, alphabet, requestId)`: Cryptographically secure random string drawn from a named alphabet. **Return**: `success(BOOL), value(STRING), length(INT), message(STRING), status(INT), data({})`
* **length**: Character count, 1..=4096. Default 32.
* **alphabet**: `"alphanumeric"` (default, A–Z a–z 0–9), `"hex"`, `"urlSafe"` (URL-safe base64 alphabet without padding), or `"pkceVerifier"` (RFC 7636 alphabet).
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `createNonce(length, requestId)`: Generates a URL-safe nonce string. **Return**: `success(BOOL), nonce(STRING), length(INT), message(STRING), status(INT), data({})`
* **length**: Character count, 8..=512. Default 32 (~192 bits at 64-char alphabet).
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `createPkceVerifier(length, requestId)`: Generates a PKCE verifier string matching RFC 7636 §4.1 alphabet and length constraints. Pair with `createPkceChallengeS256` to compute the S256 challenge. **Return**: `success(BOOL), verifier(STRING), length(INT), message(STRING), status(INT), data({})`
* **length**: 43..=128. Default 64.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `jwtDecode(token, requestId)`: Decodes a JWT into header + claims **without verifying the signature**. Use only for inspection (e.g. reading `kid` / `alg`); MUST be followed by `jwsVerify` or `jwksVerifyJwt` before trusting any claim. **Return**: `success(BOOL), header({}), claims({}), signatureEncoding(STRING), verified(BOOL), message(STRING), status(INT), data({})`
* **token**: Compact JWT string.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `jwsVerify(token, alg, key, expectedClaims, leeway, requestId)`: Verifies a JWS / JWT signature with an explicitly supplied key. Supports `HS256` (raw secret string), `RS256` (PEM RSA public key), and `ES256` (PEM EC P-256 public key). Validates `exp`/`nbf`/`iat` automatically and, when provided, `iss` / `aud` / `sub`. **Return**: `success(BOOL), valid(BOOL), claims({}), header({}), message(STRING), status(INT), data({})`
* **token**: Compact JWT string.
* **alg**: `"HS256"`, `"RS256"`, or `"ES256"`.
* **key**: HS256 secret string, or PEM-encoded public key for RS256/ES256.
* **expectedClaims**: Optional `{iss, aud, sub, nonce}` object. `aud` may be a string or array of strings.
* **leeway**: Allowed clock skew in seconds. Default 60.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `jwksResolveKey(jwksJson, kid, alg, requestId)`: Looks up a JWK by `kid` (and optionally `alg`) inside a JWKS JSON document already fetched by the caller via `ServerApi`. Returns the matching JWK plus diagnostic metadata. **Return**: `success(BOOL), found(BOOL), kid(STRING), alg(STRING), kty(STRING), use(STRING), jwk({}), message(STRING), status(INT), data({})`
* **jwksJson**: JWKS document. Either a JSON object (preferred) or a JSON string.
* **kid**: Key id to look up.
* **alg**: Optional algorithm hint; the resolved JWK is unchanged but the `alg` field in the response prefers this value.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `jwksVerifyJwt(jwksJson, token, expectedClaims, leeway, requestId)`: One-shot JWKS-based JWT verification. Reads `kid` and `alg` from the token header, looks up the matching JWK in `jwksJson`, builds a decoding key, then verifies the signature and claims. The toolkit does NOT fetch JWKS over HTTP — the caller is responsible for using VL `ServerApi` to fetch and cache the issuer's JWKS document, then passing it in. **Return**: `success(BOOL), valid(BOOL), kid(STRING), claims({}), header({}), message(STRING), status(INT), data({})`
* **jwksJson**: JWKS document (object or JSON string).
* **token**: Compact JWT string.
* **expectedClaims**: Optional `{iss, aud, sub, nonce}` object.
* **leeway**: Allowed clock skew in seconds. Default 60.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `rsaSha256Sign(data, pemKey, outputEncoding, requestId)`: Computes an RSA-PKCS#1 v1.5 signature over SHA-256(data) using a PEM-encoded PKCS#8 private key. Use this only for signing project-owned material; production private keys should come from backend `SECRET` declarations resolved through `SYSENV`. **Return**: `success(BOOL), signature(STRING), message(STRING), status(INT), data({})`
* **data**: Payload string to sign.
* **pemKey**: PEM-encoded PKCS#8 RSA private key.
* **outputEncoding**: `"hex"` (default), `"base64"`, or `"base64url"`.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `rsaSha256Verify(data, pemKey, signature, signatureEncoding, requestId)`: Verifies an RSA-PKCS#1 v1.5 signature over SHA-256(data) using a PEM-encoded RSA public key. **Return**: `success(BOOL), valid(BOOL), message(STRING), status(INT), data({})`
* **data**: Payload string that was signed.
* **pemKey**: PEM-encoded RSA public key (SubjectPublicKeyInfo).
* **signature**: Signature bytes in the encoding specified by `signatureEncoding`.
* **signatureEncoding**: `"hex"` (default), `"base64"`, or `"base64url"`.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `ecdsaP256Sha256Sign(data, pemKey, outputEncoding, requestId)`: Computes an ECDSA P-256 signature over SHA-256(data) using a PEM-encoded PKCS#8 private key. The signature is returned as fixed-size r||s (64 bytes), matching the JWS ES256 wire format. **Return**: `success(BOOL), signature(STRING), message(STRING), status(INT), data({})`
* **data**: Payload string to sign.
* **pemKey**: PEM-encoded PKCS#8 ECDSA P-256 private key.
* **outputEncoding**: `"hex"` (default), `"base64"`, or `"base64url"`.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `ecdsaP256Sha256Verify(data, pemKey, signature, signatureEncoding, requestId)`: Verifies an ECDSA P-256 signature over SHA-256(data) using a PEM-encoded EC public key. Expects 64-byte r||s signature bytes. **Return**: `success(BOOL), valid(BOOL), message(STRING), status(INT), data({})`
* **data**: Payload string that was signed.
* **pemKey**: PEM-encoded EC P-256 public key (SubjectPublicKeyInfo).
* **signature**: 64-byte r||s signature in the encoding specified by `signatureEncoding`.
* **signatureEncoding**: `"hex"` (default), `"base64"`, or `"base64url"`.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `rsaOaepSha256Encrypt(data, pemKey, inputEncoding, outputEncoding, requestId)`: Encrypts data with an RSA public key using RSA-OAEP with SHA-256. **Return**: `success(BOOL), ciphertext(STRING), message(STRING), status(INT), data({})`
* **data**: Data to encrypt.
* **pemKey**: PEM-encoded RSA public key in SubjectPublicKeyInfo format.
* **inputEncoding**: Encoding of `data`. `"raw"` (default), `"base64"`, `"base64url"`, or `"hex"`.
* **outputEncoding**: Encoding of returned ciphertext. `"base64"` (default), `"base64url"`, `"hex"`, or `"raw"`.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `rsaOaepSha256Decrypt(ciphertext, pemKey, inputEncoding, outputEncoding, requestId)`: Decrypts RSA-OAEP-SHA256 ciphertext with an RSA private key. **Return**: `success(BOOL), plaintext(STRING), message(STRING), status(INT), data({})`
* **ciphertext**: Ciphertext to decrypt.
* **pemKey**: PEM-encoded RSA private key in PKCS#8 format.
* **inputEncoding**: Encoding of `ciphertext`. `"base64"` (default), `"base64url"`, `"hex"`, or `"raw"`.
* **outputEncoding**: Encoding of returned plaintext. `"raw"` (default), `"base64"`, `"base64url"`, or `"hex"`.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.
* **security boundary**: RSA PKCS#1 v1.5 encryption is not supported. Wrong key, malformed ciphertext, and padding failures must return stable failure without exposing low-level details. Error messages must not contain PEM data, private keys, plaintext, or full ciphertext.

* `aesGcmEncrypt(plaintext, plaintextEncoding, key, keyEncoding, nonce, nonceEncoding, aad, aadEncoding, outputEncoding, requestId)`: Encrypts plaintext with AES-256-GCM and returns authenticated ciphertext. **Return**: `success(BOOL), ciphertext(STRING), nonce(STRING), algorithm(STRING), message(STRING), status(INT), data({})`
* **plaintext**: Plaintext to encrypt.
* **plaintextEncoding**: Encoding of `plaintext`. `"raw"` (default), `"base64"`, `"base64url"`, or `"hex"`.
* **key**: AES-256 key. After decoding, it must be exactly 32 bytes.
* **keyEncoding**: Encoding of `key`. `"base64"` (default), `"base64url"`, `"hex"`, or `"raw"`.
* **nonce**: AES-GCM nonce. Optional; when omitted, the toolkit generates a secure 12-byte nonce and returns it.
* **nonceEncoding**: Encoding of `nonce`. `"base64"` (default), `"base64url"`, `"hex"`, or `"raw"`.
* **aad**: Additional authenticated data. It is authenticated but not encrypted. Default empty.
* **aadEncoding**: Encoding of `aad`. `"raw"` (default), `"base64"`, `"base64url"`, or `"hex"`.
* **outputEncoding**: Encoding of returned ciphertext and nonce. `"base64"` (default), `"base64url"`, `"hex"`, or `"raw"`.
* **algorithm**: Always `"aes-256-gcm"`.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `aesGcmDecrypt(ciphertext, inputEncoding, key, keyEncoding, nonce, nonceEncoding, aad, aadEncoding, outputEncoding, requestId)`: Decrypts AES-256-GCM ciphertext and verifies the authentication tag. **Return**: `success(BOOL), plaintext(STRING), message(STRING), status(INT), data({})`
* **ciphertext**: Ciphertext to decrypt. It includes the GCM authentication tag.
* **inputEncoding**: Encoding of `ciphertext`. `"base64"` (default), `"base64url"`, `"hex"`, or `"raw"`.
* **key**: AES-256 key. After decoding, it must be exactly 32 bytes.
* **keyEncoding**: Encoding of `key`. `"base64"` (default), `"base64url"`, `"hex"`, or `"raw"`.
* **nonce**: AES-GCM nonce. After decoding, it must be exactly 12 bytes.
* **nonceEncoding**: Encoding of `nonce`. `"base64"` (default), `"base64url"`, `"hex"`, or `"raw"`.
* **aad**: Additional authenticated data. It must match the encryption call. Default empty.
* **aadEncoding**: Encoding of `aad`. `"raw"` (default), `"base64"`, `"base64url"`, or `"hex"`.
* **outputEncoding**: Encoding of returned plaintext. `"raw"` (default), `"base64"`, `"base64url"`, or `"hex"`.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.
* **security boundary**: AES-GCM decryption must fail when any of key, nonce, aad, or ciphertext does not match the encryption call. ECB, CBC, CFB, CTR, OFB, PKCS7, and other legacy block-cipher modes are not supported and must not be selected as compatibility defaults.

* `stateCreate(payload, secret, ttlSeconds, requestId)`: Builds an OAuth `state` token bound to an arbitrary JSON payload with an HMAC-SHA256 signature and an embedded expiration. This is the **stateless** form (no platform storage); use it for OAuth `start` whenever the trade-off "signed + TTL" is acceptable. A future `stateConsume` with one-time semantics will arrive once platform storage (Redis / DynamoDB) is wired. **Return**: `success(BOOL), stateToken(STRING), expiresAt(INT), issuedAt(INT), message(STRING), status(INT), data({})`
* **payload**: Arbitrary JSON object describing the OAuth context (provider, project, returnUrl, nonce, etc.). MUST NOT contain secrets — the token is base64url-encoded and inspectable by anyone who has it.
* **secret**: HMAC secret string used to sign the state. Should come from a stable platform secret, not from project user input.
* **ttlSeconds**: 30..=86400. Default 600 (10 minutes).
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `stateVerifyStateless(stateToken, secret, leeway, requestId)`: Verifies a state token previously issued by `stateCreate`. Checks the HMAC signature in constant time, then enforces `exp + leeway >= now`. Returns the original payload on success. **Return**: `success(BOOL), valid(BOOL), payload({}), issuedAt(INT), expiresAt(INT), message(STRING), status(INT), data({})`
* **stateToken**: Token previously returned by `stateCreate`.
* **secret**: Same HMAC secret used at create time.
* **leeway**: Allowed clock skew in seconds. Default 60.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `verifyHmacHeader(data, key, providedMac, algorithm, providedMacEncoding, requestId)`: Constant-time-compares an inbound webhook HMAC header against a re-computed MAC over `data`. **Return**: `success(BOOL), valid(BOOL), algorithm(STRING), message(STRING), status(INT), data({})`
* **data**: Payload bytes the sender claims to have signed.
* **key**: HMAC secret string. Production secrets should come from backend `SECRET` declarations resolved through `SYSENV`.
* **providedMac**: MAC value as received in the request header.
* **algorithm**: `"hmacSha256"` (default) or `"hmacSha1"`.
* **providedMacEncoding**: `"hex"` (default), `"base64"`, or `"base64url"`.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

* `verifyTimestampedHmacHeader(data, key, providedMac, algorithm, providedMacEncoding, timestamp, toleranceSeconds, separator, requestId)`: Like `verifyHmacHeader` but also enforces a freshness window on `timestamp` to defeat replay attacks (Stripe-style `t=.,v1=...` flow). The signed payload is `timestamp + separator + data`; the default separator is `"."` and works for Stripe; pass `"\n"` for Slack-style flows. **Return**: `success(BOOL), valid(BOOL), algorithm(STRING), drift(INT), reason(STRING), message(STRING), status(INT), data({})`
* **data**: Payload bytes the sender signed.
* **key**: HMAC secret string. Production secrets should come from backend `SECRET` declarations resolved through `SYSENV`.
* **providedMac**: MAC value from the request header.
* **algorithm**: `"hmacSha256"` (default) or `"hmacSha1"`.
* **providedMacEncoding**: `"hex"` (default), `"base64"`, or `"base64url"`.
* **timestamp**: Unix epoch seconds from the request header.
* **toleranceSeconds**: Maximum allowed `|now - timestamp|`. Default 300.
* **separator**: String inserted between `timestamp` and `data` when computing the expected MAC. Default `"."`. Pass `""` to concatenate directly.
* **requestId**: Optional trace id. Pass an empty string when no trace id is needed.

**Stable Error Codes**

New security methods must use stable error codes:

| Code | Meaning |
| ---- | ------- |
| `INVALID_INPUT` | Missing required parameter, invalid key or nonce length, PEM parse failure, malformed ciphertext, or invalid encoding input |
| `UNSUPPORTED_ENCODING` | Encoding value is not one of `"raw"`, `"base64"`, `"base64url"`, or `"hex"` where applicable |
| `SIGNATURE_INVALID` | Decryption authentication failure, wrong key, wrong ciphertext, wrong nonce, wrong aad, invalid signature, or equivalent verification failure |
| `INTERNAL_ERROR` | Internal encryption, decryption, signing, or verification failure |

Error messages must not expose secrets, private keys, plaintext, complete ciphertext, DSNs, or low-level panic details.

**Notes**

* `BackendSecurityToolkit` is backend-only and belongs in `# Backend Tree`.
* The component exposes platform primitives only. Project business authentication, user creation, and login token issuance remain separate concerns.
* Provider credentials and project secrets should be declared as backend `SECRET` values, resolved to backend `SYSENV`, and then passed only to backend security methods that require them.
...
#### Encryption (Legacy Cryptographic Compatibility)

`Encryption` / `data-encrypt` is a legacy compatibility entry. New VL code should use `BackendSecurityToolkit` / `server-security`.

The legacy entry may route through a strict-modern adapter while preserving the old return envelope shape `{status, result}`. The adapter supports only the modern security subset:
...
Unsafe or semantically unclear legacy capabilities must be rejected with stable failures rather than silently mapped to unsafe defaults.

Known legacy capabilities that are not supported as default compatibility behavior:

| Legacy capability | Current rule |
| ----------------- | ------------ |
| AES-CBC / CFB / CTR / OFB / ECB | Not supported by default; handle only by a separate project-specific compatibility task |
| RSA PKCS#1 v1.5 encryption | Not supported; use RSA-OAEP-SHA256 |
| SM2 / SM3 / SM4 | Not supported by default; handle only by compliance or customer-specific requirements |
| `genHashCode` / `verifyHashCode` | Not implemented until the old authoritative semantics are available |
| Static component property access such as `$sys.get(node, id, ...)` | Not read by the adapter; evaluate only when a real project dependency requires it |
